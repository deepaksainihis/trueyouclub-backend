                    ...
                    <div class="search-table-data">
                        <div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ Carbon\Carbon::parse($content->scheduled_date)->format('M d, Y') }}</td>
                                    <td><span class="badge bg-info" style="background-color: #E6E6FA !important;">{{ ucfirst(str_replace('_', ' ', $content->type)) }}</span></td>
                                    <td>{{ $content->title }}</td>
                                    <td>
                                        <div class="update-webinar table-btns">
                                            <ul class="d-flex">
                                                <li>
                                                    <a href="javascript:void(0)" wire:click="edit({{ $content->id }})">
                                                ...
                                                <li>
                                                    <a href="javascript:void(0)" wire:click="delete({{ $content->id }})">
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="text-center">No content scheduled yet.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <form wire:submit.prevent="{{ $updateMode ? 'update' : 'store' }}" class="forms-sample" enctype="multipart/form-data">
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="date" class="form-control" wire:model.defer="scheduled_date">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control" wire:model="type">
                                <option value="micro_learning">Micro Learning (Text)</option>
                                <option value="audio">Audio Affirmation</option>
                        ...
                        <div class="form-group mb-3">
                            <label>Title</label>
                            <input type="text" class="form-control" wire:model.defer="title" placeholder="e.g. Morning Motivation">
                        ...
                        <div class="form-group mb-3">
                            ...
                            @error('new_media') <span class="text-danger">{{ $message }}</span> @enderror
                            
                            @if($media_path && !$new_media)
                                <div class="mt-2 text-success">Current file uploaded.</div>
                            @endif
                        </div>
                        @endif
                    </div>
                    <div class="modal-footer">
                        ...
                        <button wire:click.prevent="cancel" class="btn btn-secondary" data-bs-dismiss="modal">
                            Cancel
                        </button>
                    </div>
                </form>
...
<script type="text/javascript">
    document.addEventListener('openModal', function(event) {
        $('#daily_content_modal').modal('show');
    });
    Livewire.on('refreshTable', () => {
        $('#daily_content_modal').modal('hide');
    });
</script>
@endpush
